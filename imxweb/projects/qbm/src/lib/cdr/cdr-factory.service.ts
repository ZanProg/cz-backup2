...
public buildCdrFromColumnList2(entity: IEntity, columnNames: string[], readOnlyColumns: string[] = [], defaultValues: Record<string, any> = {}): ColumnDependentReference[] {
  return columnNames
    .map((column) => {
      // Create and return the CDR object
      return this.buildCdr(entity, column, readOnlyColumns.includes(column), undefined, defaultValues[column]);
    })
    .filter((cdr) => cdr != null);
}
...
public buildCdrFromColumnList3(entity: IEntity, columnNames: string[], readOnlyColumns: string[] = [], defaultValues: Record<string, any> = {}): ColumnDependentReference[] {
   ...
   */
  public buildCdr(entity: IEntity, columnName: string, readOnly: boolean = false, columnDisplay?: string, defaultValue?: any): ColumnDependentReference {
