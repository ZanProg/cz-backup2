...
export class CartItemsService {
  ...
  public async createCartandSubmit(requestableServiceItemsForPersons: RequestableProductForPerson[], columnsToSave: string): Promise<void> {
    this.addItems(requestableServiceItemsForPersons, columnsToSave);
  }
  ...
  public async createAndPost(
    ...
    cartItem.UID_PersonOrdered.value = requestableServiceItemForPerson.UidPerson;
    cartItem.UID_ITShopOrg.value = requestableServiceItemForPerson.UidITShopOrg;
    cartItem.CustomProperty10.value = CustomProperty10;
    if (requestableServiceItemForPerson?.UidPatternItem?.length > 0) {
      cartItem.UID_PatternItem.value = requestableServiceItemForPerson.UidPatternItem;
    }
    if (parentCartUid) {
  ...
  public async addItems(requestableServiceItemsForPersons: RequestableProduct[], CustomProperty10?: string): Promise<number> {
    const addedItems: PortalCartitem[] = [];
    const cartitemReferences: string[] = [];
    const cartItemsWithoutParams: PortalCartitem[] = [];
    ...
    for await (const requestable of requestableServiceItemsForPersons) {
      ...
      const cartItemCollection = await this.createAndPost(requestable, parentCartUid, CustomProperty10);
      ...
      if (CustomProperty10.length > 0) {
        this.submit(cartItemCollection.Data[0].UID_ShoppingCartOrder.value, CheckMode.SubmitWithWarnings);
      }
    }
