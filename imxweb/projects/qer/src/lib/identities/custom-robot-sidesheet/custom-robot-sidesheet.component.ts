...
@Component({
  selector: 'ccc-custom-robot-sidesheet',
  templateUrl: './custom-robot-sidesheet.component.html',
  styleUrls: ['./custom-robot-sidesheet.component.scss']
})
export class CustomRobotSidesheetComponent implements OnInit, OnDestroy {
...
this.personalColumns = this.data.projectConfig.OwnershipConfig.EditableFields["Person"];
...
const organizationalColumns = this.data.projectConfig.PersonConfig.VI_Employee_MasterData_OrganizationalAttributes;
   ...
   const customColumns = [
    ...
    this.cdrListPersonal = this.cdrFactoryService.buildCdrFromColumnList(this.data.selectedIdentity.GetEntity(), this.personalColumns, !this.data.canEdit);
...
this.cdrListOrganizational = this.cdrFactoryService.buildCdrFromColumnList(this.data.selectedIdentity.GetEntity(), customColumns, !this.data.canEdit);

// comment out not used form part
